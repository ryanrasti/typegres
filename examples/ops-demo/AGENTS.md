...
13. **`set()` validator rejects typegres expressions.** `UpdateBuilder.set()` is `@tool`-validated by `isSetRow`, which fails any class instance — including a perfectly good typegres expression like `p.on_hand["+"]("5")` (would compile to SQL `SET on_hand = on_hand + 5`). Means you can't write `SET col = col + 1` at all from JS. Demo workaround: read-then-write inside a transaction, computing the new value in JS as a primitive. Real fix: `isSetRow` should accept Any-typed expressions OR introduce a separate `setExpr()` API.
