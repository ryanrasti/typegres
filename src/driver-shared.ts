// Shared pieces of the Driver contract + sqlite row normalization.
// Kept free of optional peer imports (pg / better-sqlite3 / pglite) so
// workerd / browser bundles can pull DoSqliteDriver without resolving
// node-only packages.
...
// stays as-is (the leading `(` closes early and re-opens). Both sqlite
// drivers need it: QueryBuilder.bind() wraps statements in `(...)` for
// subquery splicing, and SQLite refuses top-level parenthesized statements.
