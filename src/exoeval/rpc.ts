...
export class RpcClient<A> {
    ...
    async *#stream(code: string): AsyncIterable<unknown> {
        for await (const chunk of this.channel(code)) {
            // sjson rejects `__proto__` / `constructor` keys (same parser
            // exoeval exposes as `JSON.parse` to evaluated code). The wire
            // is untrusted; raw JSON.parse here would let a malicious
            // payload pollute Object.prototype on the client.
            yield sjson.parse(chunk);
        }
    }
}
...
export const safeStringify = (value: unknown): string => {
  ...
  const out = JSON.stringify(value, (_key, val) => {
    if (!isPlainData(val)) {
      const name = (Object.getPrototypeOf(val) as { constructor?: { name?: string } } | null)?.constructor?.name ?? "anonymous";
      throw new TypeError(
  ...
  // Past the top-level `undefined` guard, the only way JSON.stringify
  // still returns undefined is if `value` is a function or a symbol —
  // those shouldn't reach the wire, so surface the bug rather than
  // silently sending "null".
  if (typeof out !== "string") {
    throw new TypeError(
      `safeStringify: cannot serialize ${typeof value} over RPC.`,
    );
  }
  return out;
};
