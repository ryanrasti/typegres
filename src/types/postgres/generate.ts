...
const generateTypeFile = (
  ...
  //
  // The root `any` type extends the shared dialect-agnostic `SqlValue`
  // base (which provides cast/coalesce/from/serialize/column/deserialize
  // /isNull/isNotNull/in). Everything else chains down through the pg_catalog
  // hierarchy to `any`, so those methods are inherited transitively.
  ...
  if (pgType.typname === "any") {
    lines.push('import { SqlValue } from "../../sql-value";');
  } else if (parentTypname) {
  ...
  if (pgType.typname === "any") {
    ...
    // Concrete type — narrow [meta]. For classes with no hand-written
    // override under overrides/, emit a `declare deserialize` return-type
    // marker so `TsTypeOf<Cls<1>>` narrows (SqlValue.deserialize returns
    // `unknown`). For classes with an override, the override provides
    // both the runtime parser AND the narrower return type — codegen
    // skips the declare to avoid a contradictory signature.
  ...
  const buildBody = (funcs: PgFunc[], allowMap: Map<PgFunc, boolean>): string => {
    ...
    const caller = f0.isSrf
      ? `new runtime.Srf("${f0.name}", [this, ...__rest], [["${f0.name}", __rt]])`
      : f0.isOperator
        ? `runtime.opCall(runtime.sql\`${f0.name}\`, [this, ...__rest] as [unknown, unknown], __rt)`
        : `runtime.funcCall("${f0.name}", [this, ...__rest], __rt)`;
  ...
  const wrapSrfRet = (f: PgFunc, retType: string): string =>
    `runtime.Srf<{ ${f.name}: ${retType} }, "${f.name}">`;
  ...
  for (const group of allGroups) {
    ...
    if (f0.isSrf && f0.outColumns && f0.outColumns.length > 0) {
      ...
      const ret = `runtime.Srf<{ ${colEntries.join("; ")} }, "${f0.name}">`;
      lines.push(`  @expose.unchecked()`);
      lines.push(`  ${sig}: ${ret} { return new runtime.Srf("${f0.name}", [${allArgs}], [${colRuntime.join(", ")}]) as any; }`);
      continue;
    }
