...
export default class Bool<N extends number> extends PgBool<N> {
  ...
  private toBoolExpression(value: Types.Bool<any> | Types.Input<Types.Bool<any>>): Expression {
    return value instanceof Types.Any ? value.toExpression() : Types.Bool.new(value as boolean).toExpression();
  }
   ...
   */
  and<N2 extends number>(other: Types.Bool<N2> | Types.Input<Types.Bool<N2>>): Types.Bool<N | N2> {
    return Types.Bool.new(
      new BinaryOperatorExpression("AND", [this.toExpression(), this.toBoolExpression(other)]),
    ) as Types.Bool<N | N2>;
  }
   ...
   */
  or<N2 extends number>(other: Types.Bool<N2> | Types.Input<Types.Bool<N2>>): Types.Bool<N | N2> {
    return Types.Bool.new(
      new BinaryOperatorExpression("OR", [this.toExpression(), this.toBoolExpression(other)]),
    ) as Types.Bool<N | N2>;
  }
